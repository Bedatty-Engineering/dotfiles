...
if grep -qi microsoft /proc/version 2>/dev/null; then
  echo "  ${C_YELLOW}WSL detected.${C_RESET} The Linux builds were installed for completeness, but on WSL you"
  echo "  should run the Windows builds in ${C_CYAN}Remote-WSL${C_RESET} mode (cursor-server runs inside Linux):"
  ...
  echo "      4. ${C_BOLD}From each Windows app, open Command Palette (Ctrl+Shift+P) and run:${C_RESET}"
  echo "         ${C_CYAN}Shell Command: Install 'code' command in PATH${C_RESET}"
  echo "         ${C_CYAN}Shell Command: Install 'cursor' command in PATH${C_RESET}"
  echo "      5. From WSL: ${C_CYAN}code .${C_RESET} / ${C_CYAN}cursor .${C_RESET} — opens in Remote-WSL automatically"
  echo "  ${C_DIM}Note: do NOT alias these to .exe — that bypasses Remote-WSL mode${C_RESET}"
else
